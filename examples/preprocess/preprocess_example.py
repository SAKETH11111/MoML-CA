...
from code.MGNN.utils.config import MGNNConfig
...
def main():
    ...
    parser.add_argument('--file_pattern', type=str, default='*.mol',
    ...
    for file_path in tqdm(mol_files):
        try:
            ...
            output_file = os.path.join(
                args.output_dir, 
                os.path.basename(file_path).split('.')[0] + '_graph.pt'
            )
    ...
    if processed_files > 0:
        ...
        stats_df = pd.DataFrame({
            'statistic': [
                'num_files', 
                'avg_nodes', 'min_nodes', 'max_nodes', 'std_nodes',
                'avg_edges', 'min_edges', 'max_edges', 'std_edges',
                'node_feature_dim', 'edge_feature_dim'
            ],
            'value': [
                processed_files,
                np.mean(feature_stats['num_nodes']), np.min(feature_stats['num_nodes']), 
                np.max(feature_stats['num_nodes']), np.std(feature_stats['num_nodes']),
                np.mean(feature_stats['num_edges']), np.min(feature_stats['num_edges']), 
                np.max(feature_stats['num_edges']), np.std(feature_stats['num_edges']),
                feature_stats['node_feature_dim'], feature_stats['edge_feature_dim']
            ]
        })
        ...
        print(f"Nodes: avg={np.mean(feature_stats['num_nodes']):.1f}, "
             f"min={np.min(feature_stats['num_nodes'])}, "
             f"max={np.max(feature_stats['num_nodes'])}")
        print(f"Edges: avg={np.mean(feature_stats['num_edges']):.1f}, "
