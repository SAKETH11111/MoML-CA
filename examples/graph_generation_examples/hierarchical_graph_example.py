...
def parse_args():
    ...
    parser.add_argument("--mol_file", type=str, required=True, help="Path to the molecule file (MOL/SDF format)")
    parser.add_argument(
        "--orca_file", type=str, default=None, help="Path to the ORCA output file (optional, for quantum properties)"
    )
    parser.add_argument("--output_dir", type=str, default="./output", help="Directory to save the hierarchical graphs")
    parser.add_argument("--visualize", action="store_true", help="Whether to visualize the hierarchical graphs")
    parser.add_argument("--use_3d", action="store_true", help="Whether to use 3D coordinates in the graphs")
...
def main():
    ...
    coarsener = GraphCoarsener(use_3d_coords=args.use_3d, use_pfas_features=True)  # Enable PFAS-specific features
    ...
    if args.orca_file is None:
        ...
        graph_paths = coarsener.create_from_molecule_file(mol_file=args.mol_file, output_dir=args.output_dir)
