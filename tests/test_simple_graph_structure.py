...
try:
    ...
    print(f"Failed to import MolecularGraphProcessor: {e}")
...
class TestSimpleMolecularGraph(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Initialize the MolecularGraphProcessor if available
        if MGNN_AVAILABLE:
            self.graph_processor = create_graph_processor(
                {"use_partial_charges": False, "use_3d_coords": True, "use_pfas_specific_features": True}
            )
    ...
    def test_graph_creation(self):
        ...
        if not MGNN_AVAILABLE:
            self.skipTest("MolecularGraphProcessor not available")
        ...
        print(
            f"Basic graph creation passed for ethane with {graph.num_nodes} atoms and "
    ...
    def test_pfas_graph(self):
        ...
        print(
            f"PFAS graph creation passed for trifluoromethane with {graph.num_nodes} atoms, "
    ...
    def test_complex_pfas(self):
        ...
        print(
            f"Complex PFAS graph creation passed for GenX with {graph.num_nodes} atoms, "
            f"including {f_count} fluorine atoms"
        )
    ...
    def test_torch_conversion(self):
        ...
        if not TORCH_AVAILABLE or not MGNN_AVAILABLE:
            self.skipTest("PyTorch or MolecularGraphProcessor not available")
        ...
        print("PyTorch tensor conversion passed")
