...
from moml.models.mgnn.djmgnn import DenseGNNBlock, JKAggregator # HMGNN uses these
    ...
    if is_batch:
        ...
        for _ in range(batch_size):
            instance_data = _create_hierarchical_graph_data(
                num_scales, nodes_counts, edges_counts, 
                scale_node_dims, scale_edge_attr_dims, 
...
def dummy_hierarchical_graph_data_single(request) -> List[Dict[str, Any]]:
    ...
    return _create_hierarchical_graph_data(
        NUM_SCALES, nodes_counts, EDGES_COUNTS_PER_SCALE, 
        SCALE_NODE_DIMS_HMGNN, edge_attr_dims
    )
...
def dummy_hierarchical_graph_data_batch(request) -> List[Dict[str, Any]]:
    edge_attr_dims = getattr(request, "param", {}).get("edge_attr_dims", SCALE_EDGE_ATTR_DIMS_PRESENT)
    return _create_hierarchical_graph_data(
        NUM_SCALES, NODES_COUNTS_PER_SCALE, EDGES_COUNTS_PER_SCALE,
        SCALE_NODE_DIMS_HMGNN, edge_attr_dims,
        is_batch=True, batch_size=BATCH_SIZE_HMGNN
    )
...
class TestHMGNN:
    ...
    def test_instantiation(self):
        model = HMGNN(
            scale_dims=SCALE_NODE_DIMS_HMGNN, hidden_dim=HIDDEN_DIM_HMGNN,
            n_blocks=self.N_BLOCKS_HMGNN, layers_per_block=self.LAYERS_PER_BLOCK_HMGNN,
            edge_attr_dims=SCALE_EDGE_ATTR_DIMS_PRESENT,
            node_out_dim=self.NODE_OUT_DIM_HMGNN, graph_out_dim=self.GRAPH_OUT_DIM_HMGNN,
    ...
    def test_instantiation_edge_attr_none(self):
        model = HMGNN(
            scale_dims=SCALE_NODE_DIMS_HMGNN, hidden_dim=HIDDEN_DIM_HMGNN,
            edge_attr_dims=None # Test default behavior
        )
    ...
    @pytest.mark.parametrize("dummy_hierarchical_graph_data_single", [{"edge_attr_dims": SCALE_EDGE_ATTR_DIMS_PRESENT}], indirect=True)
    ...
    @pytest.mark.parametrize("dummy_hierarchical_graph_data_batch", [{"edge_attr_dims": SCALE_EDGE_ATTR_DIMS_PRESENT}], indirect=True)
    ...
    @pytest.mark.parametrize("dummy_hierarchical_graph_data_single", [{"edge_attr_dims": SCALE_EDGE_ATTR_DIMS_ABSENT}], indirect=True)
    def test_forward_pass_single_graph_no_edge_attr(self, dummy_hierarchical_graph_data_single, dummy_cluster_mappings):
        model = HMGNN(
            scale_dims=SCALE_NODE_DIMS_HMGNN, hidden_dim=HIDDEN_DIM_HMGNN,
            edge_attr_dims=SCALE_EDGE_ATTR_DIMS_ABSENT, cross_scale_exchange=False
        )
    ...
    def test_forward_pass_single_scale(self, dummy_hierarchical_graph_data_single):
        ...
        model = HMGNN(
            scale_dims=single_scale_dims, hidden_dim=HIDDEN_DIM_HMGNN,
            edge_attr_dims=single_edge_dims, cross_scale_exchange=False
        )
    ...
    @pytest.mark.parametrize("dummy_hierarchical_graph_data_single", [{"nodes_counts": [NUM_NODES_SCALE_0, 0, NUM_NODES_SCALE_2]}], indirect=True)
    def test_forward_pass_zero_nodes_in_one_scale(self, dummy_hierarchical_graph_data_single, dummy_cluster_mappings):
        ...
        model = HMGNN(
            scale_dims=SCALE_NODE_DIMS_HMGNN, hidden_dim=HIDDEN_DIM_HMGNN,
            edge_attr_dims=SCALE_EDGE_ATTR_DIMS_PRESENT, cross_scale_exchange=True
        )
