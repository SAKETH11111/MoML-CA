...
def mock_atom_graph(test_molecule):
    """
    Create a mock atom-level graph for testing, avoiding RDKit bugs.
    This is a simplified version without using MolecularGraphBuilder.
    """
    mol = test_molecule
...
class TestFunctionalGroupIdentifier:
    ...
    def test_identify_cf_groups(self, test_molecule):
        ...
        cf_groups = identifier.identify_cf_groups(test_molecule)
    ...
    def test_identify_carboxylic_groups(self, test_molecule):
        ...
        carboxylic_groups = identifier.identify_carboxylic_groups(test_molecule)
    ...
    def test_identify_all_functional_groups(self, test_molecule):
        ...
        cf_groups, functional_groups = identifier.identify_all_functional_groups(test_molecule)
...
class TestGraphCoarsener:
    ...
    @patch('code.MGNN.architectures.molecular_graph.MolecularGraphBuilder._calculate_distance_features')
    def test_create_structural_motif_graph(self, mock_calc_dist, mock_functional_group_graph, test_molecule):
        ...
        """
        # Mock the _calculate_distance_features method to avoid RDKit errors
        mock_calc_dist.return_value = {
            i: {'dist_to_cf3': 0.5, 'dist_to_functional': 0.5} 
...
class TestGraphCoarseningUtils:
    """Tests for graph coarsening utility functions."""
    
    def test_utility_functions_availability(self):
        """Test that utility functions are available and callable."""
        # These functions need specialized environments and mocks to test fully,
        # but we can at least check they're available and callable
        assert callable(create_hierarchical_graphs_from_orca)
        assert callable(batch_create_hierarchical_graphs)
        assert callable(visualize_graph_hierarchy)
