...
__all__ = [
    'MolecularGraphProcessor', 'create_graph_processor', 'mol_file_to_graph',
    'graph_to_device', 'collate_graphs', 'find_charges_file',
    'read_charges_from_file', 'create_molecular_graph_json',
...
class MolecularGraphProcessor:
    ATOM_FEATURES_DEFAULTS = {
        ...
        'hybridization': [
            ...
            Chem.rdchem.HybridizationType.SP3D2, Chem.rdchem.HybridizationType.UNSPECIFIED, -1
        ],
        'is_aromatic': [0, 1], 'is_in_ring': [0, 1],
        'num_hydrogens': None, 'is_fluorine': None, 'is_carbon_bonded_to_fluorine': None,
        'num_fluorine_neighbors': None, 'is_in_carboxylic_group': None,
        'is_in_sulfonic_group': None, 'is_in_phosphonic_group': None,
        'partial_charge': None, 'dist_to_cf3': None, 'dist_to_functional_group': None,
        'is_head_group_atom': None, 'homo_contribution': None, 'lumo_contribution': None,
    }

    BOND_FEATURES_DEFAULTS = {
        'bond_type': [
            Chem.rdchem.BondType.SINGLE, Chem.rdchem.BondType.DOUBLE,
            Chem.rdchem.BondType.TRIPLE, Chem.rdchem.BondType.AROMATIC, -1
        ],
        'is_conjugated': [0, 1], 'is_in_ring': [0, 1],
        'is_cf_bond': None, 'is_cf_cf_bond': None, 'is_fluorinated_tail_bond': None,
        'is_functional_group_bond': None, 'bond_length': None,
    }
    ...
    def atom_feature_dim(self) -> int:
        ...
        for scheme in self.atom_feature_schemes:
            if scheme not in self.ATOM_FEATURES_DEFAULTS: continue
            choices = self.ATOM_FEATURES_DEFAULTS[scheme]
            if isinstance(choices, list): dim += len(choices)
            else:
                if scheme == 'partial_charge' and not self.use_partial_charges: continue
                if scheme in ['dist_to_cf3', 'dist_to_functional_group', 'is_head_group_atom'] and \
                   not (self.use_pfas_specific_features and self.use_3d_coords): continue
                if scheme in ['num_fluorine_neighbors', 'is_in_carboxylic_group',
                              'is_in_sulfonic_group', 'is_in_phosphonic_group'] and \
                   not self.use_pfas_specific_features: continue
                if scheme in ['homo_contribution', 'lumo_contribution']: continue
                dim += 1
        return dim
    ...
    def bond_feature_dim(self) -> int:
        ...
        for scheme in self.bond_feature_schemes:
            if scheme not in self.BOND_FEATURES_DEFAULTS: continue
            choices = self.BOND_FEATURES_DEFAULTS[scheme]
            if isinstance(choices, list): dim += len(choices)
            else:
                if scheme == 'bond_length' and not self.use_3d_coords: continue
    ...
    def _one_hot_encoding(value: Any, choices: List[Any]) -> List[int]:
        ...
        try:
            ...
            if choices and choices[-1] in [-1, -999]: encoding[-1] = 1
        return encoding
    ...
    def _get_atom_features(self, atom: Chem.Atom, mol: Chem.Mol,
        ...
        for scheme in self.atom_feature_schemes:
            if scheme == 'atomic_num': features.extend(self._one_hot_encoding(atom.GetAtomicNum(), self.ATOM_FEATURES_DEFAULTS['atomic_num']))
            elif scheme == 'degree': features.extend(self._one_hot_encoding(atom.GetDegree(), self.ATOM_FEATURES_DEFAULTS['degree']))
            elif scheme == 'formal_charge': features.extend(self._one_hot_encoding(atom.GetFormalCharge(), self.ATOM_FEATURES_DEFAULTS['formal_charge']))
            elif scheme == 'hybridization': features.extend(self._one_hot_encoding(atom.GetHybridization(), self.ATOM_FEATURES_DEFAULTS['hybridization']))
            elif scheme == 'is_aromatic': features.extend(self._one_hot_encoding(1 if atom.GetIsAromatic() else 0, self.ATOM_FEATURES_DEFAULTS['is_aromatic']))
            elif scheme == 'is_in_ring': features.extend(self._one_hot_encoding(1 if atom.IsInRing() else 0, self.ATOM_FEATURES_DEFAULTS['is_in_ring']))
            elif scheme == 'num_hydrogens': features.append(atom.GetTotalNumHs())
            elif scheme == 'is_fluorine': features.append(float(is_f_atom))
            elif scheme == 'is_carbon_bonded_to_fluorine': features.append(float(is_cf_atom))
            elif scheme == 'num_fluorine_neighbors':
                 if self.use_pfas_specific_features: features.append(float(num_f_neighbors_atom))
            elif scheme == 'is_in_carboxylic_group':
                if self.use_pfas_specific_features: features.append(float(self._is_in_carboxylic_group(atom)))
            elif scheme == 'is_in_sulfonic_group':
                if self.use_pfas_specific_features: features.append(float(self._is_in_sulfonic_group(atom)))
            elif scheme == 'is_in_phosphonic_group':
                if self.use_pfas_specific_features: features.append(float(self._is_in_phosphonic_group(atom)))
            elif scheme == 'partial_charge':
                if self.use_partial_charges: features.append(partial_charge_val if partial_charge_val is not None else 0.0)
            elif scheme == 'dist_to_cf3':
                if self.use_pfas_specific_features and self.use_3d_coords: features.append(float(atom_dist_feats.get('dist_to_cf3', -1.0)))
            elif scheme == 'dist_to_functional_group':
                if self.use_pfas_specific_features and self.use_3d_coords: features.append(float(atom_dist_feats.get('dist_to_functional', -1.0)))
            elif scheme == 'is_head_group_atom':
                if self.use_pfas_specific_features and self.use_3d_coords: features.append(float(atom_dist_feats.get('is_head_group', 0.0)))
            elif scheme == 'homo_contribution':
                if homo_lumo_contrib_val is not None and len(homo_lumo_contrib_val) > 0: features.append(homo_lumo_contrib_val[0])
            elif scheme == 'lumo_contribution':
                if homo_lumo_contrib_val is not None and len(homo_lumo_contrib_val) > 1: features.append(homo_lumo_contrib_val[1])
        return features
    ...
    def _get_bond_features(self, bond: Chem.Bond, bond_lengths_map: Optional[Dict[Tuple[int, int], float]] = None) -> List[float]:
        ...
        if self.use_pfas_specific_features and begin_atom.GetAtomicNum() == 6 and end_atom.GetAtomicNum() == 6:
            is_cf_cf_bond_val = sum(1 for n in begin_atom.GetNeighbors() if n.GetAtomicNum() == 9) > 0 and \
                                sum(1 for n in end_atom.GetNeighbors() if n.GetAtomicNum() == 9) > 0
        ...
        if self.use_pfas_specific_features:
            is_func_group_bond_val = any(fn(atm) for fn in [self._is_in_carboxylic_group, self._is_in_sulfonic_group, self._is_in_phosphonic_group] for atm in [begin_atom, end_atom])
        bond_len_val = None
        ...
        for scheme in self.bond_feature_schemes:
            if scheme == 'bond_type': features.extend(self._one_hot_encoding(bond.GetBondType(), self.BOND_FEATURES_DEFAULTS['bond_type']))
            elif scheme == 'is_conjugated': features.extend(self._one_hot_encoding(1 if bond.GetIsConjugated() else 0, self.BOND_FEATURES_DEFAULTS['is_conjugated']))
            elif scheme == 'is_in_ring': features.extend(self._one_hot_encoding(1 if bond.IsInRing() else 0, self.BOND_FEATURES_DEFAULTS['is_in_ring']))
            elif scheme == 'is_cf_bond': features.append(float(is_cf_bond_val))
            elif scheme == 'is_cf_cf_bond':
                if self.use_pfas_specific_features: features.append(float(is_cf_cf_bond_val))
            elif scheme == 'is_fluorinated_tail_bond':
                if self.use_pfas_specific_features: features.append(float(is_fluorinated_tail_bond_val))
            elif scheme == 'is_functional_group_bond':
                if self.use_pfas_specific_features: features.append(float(is_func_group_bond_val))
            elif scheme == 'bond_length':
                if self.use_3d_coords and bond_len_val is not None: features.append(bond_len_val)
        return features
    ...
    def mol_to_graph(self, 
        ...
        try:
            ...
            mol_with_hs_attempt = Chem.AddHs(mol_processed, explicitOnly=False, addCoords=(mol_processed.GetNumConformers() > 0))
            ...
            if mol_with_hs_attempt.GetNumAtoms() > initial_atom_count or \
               (initial_atom_count > 0 and Descriptors.HeavyAtomCount(mol_processed) < initial_atom_count) or \
               (initial_atom_count == 0 and mol_with_hs_attempt.GetNumAtoms() > 0) : # handles H2, or if it was already all H
                ...
                logger.info(f"AddHs with addCoords=True did not increase atom count for {original_smiles_for_log}. Trying AddHs with addCoords=False.")
            ...
            logger.warning(f"Failed to add/process hydrogens for molecule: {original_smiles_for_log}. Error: {e}. Proceeding with mol_processed in its current state (atom count: {mol_processed.GetNumAtoms()}).")
        ...
        if self.use_3d_coords and mol_processed.GetNumConformers() == 0:
            try:
                ...
                if mol_processed.GetNumConformers() == 0:
                    logger.warning(f"Failed to generate 3D coordinates for {Chem.MolToSmiles(mol_processed)}. Creating dummy 2D-like conformer.")
                ...
                logger.error(f"Error generating 3D conformer for {Chem.MolToSmiles(mol_processed)}: {e}. Proceeding with current conformer state.")
        ...
        bond_lengths_map = self.feature_extractor.calculate_bond_lengths(mol_processed) if self.use_3d_coords and mol_processed.GetNumConformers() > 0 else {}
    ...
    def file_to_graph(self, file_path: str, additional_features: Optional[Dict[str, List[float]]] = None) -> Optional[Data]:
        ...
        try:
            mol = None
            if file_path.endswith('.sdf'): suppl = Chem.SDMolSupplier(file_path, removeHs=False); mol = next(iter(suppl), None)
            elif file_path.endswith('.mol2'): mol = Chem.MolFromMol2File(file_path, removeHs=False)
            elif file_path.endswith('.pdb'): mol = Chem.MolFromPDBFile(file_path, removeHs=False)
            elif file_path.endswith('.mol'): mol = Chem.MolFromMolFile(file_path, removeHs=False)
            else: logger.error(f"Unsupported molecule file format: {file_path}"); return None

            if mol is None: logger.error(f"Failed to read molecule from {file_path}"); return None
    ...
    def mol_to_json_graph(self, mol: Chem.Mol) -> Dict[str, Any]:
        ...
        for atom in mol_with_hs.GetAtoms():
            nodes.append({
                'id': atom.GetIdx(), 'atomic_num': atom.GetAtomicNum(), 'symbol': atom.GetSymbol(),
                'formal_charge': atom.GetFormalCharge(), 'hybridization': str(atom.GetHybridization()),
                'is_aromatic': atom.GetIsAromatic(), 'num_hydrogens': atom.GetTotalNumHs(),
    ...
    def _get_molecule_descriptors(self, mol: Chem.Mol) -> Dict[str, float]:
        return {
            ...
            'num_h_acceptors': Lipinski.NumHAcceptors(mol), 'num_h_donors': Lipinski.NumHDonors(mol),
    ...
    def file_to_json_graph(self, file_path: str, output_dir: Optional[str] = None, output_filename: Optional[str] = None) -> Optional[str]:
        if not os.path.exists(file_path): logger.error(f"Molecule file not found: {file_path}"); return None
        try:
            mol = None
            if file_path.endswith('.sdf'): suppl = Chem.SDMolSupplier(file_path, removeHs=False); mol = next(iter(suppl), None)
            elif file_path.endswith('.mol2'): mol = Chem.MolFromMol2File(file_path, removeHs=False)
            ...
            if not output_filename: output_filename = f"{os.path.splitext(os.path.basename(file_path))[0]}_graph.json"
...
def mol_file_to_graph(mol_file_path: str,
                      ...
                      additional_features: Optional[Dict[str, List[float]]] = None) -> Optional[Data]:
    try:
        processor = create_graph_processor(config)
        return processor.file_to_graph(mol_file_path, additional_features)
    except FileNotFoundError: return None 
    except Exception as e: logger.error(f"Failed to convert {mol_file_path} to graph: {e}"); return None

def graph_to_device(graph: Union[Data, Dict[str, torch.Tensor]], device: torch.device) -> Union[Data, Dict[str, torch.Tensor]]:
    if isinstance(graph, Data): return graph.to(device)
    elif isinstance(graph, dict): return {level: g.to(device) for level, g in graph.items()}
    return graph

def collate_graphs(graphs: List[Data]) -> Data:
    from torch_geometric.loader import DataLoader 
    if not graphs: return Batch()
...
def read_charges_from_file(charge_file: str) -> Optional[List[float]]:
    charges = []
    try:
        with open(charge_file, 'r') as f: lines = f.readlines()
        file_ext = os.path.splitext(charge_file)[1].lower()

        if file_ext in ['.charges', '.chg']:
            for line in lines:
                try: charges.append(float(line.split()[-1]))
                except (ValueError, IndexError): logger.warning(f"Could not parse charge from line: '{line.strip()}' in {charge_file}")
        elif file_ext == '.txt': # ORCA Mulliken or similar
            for line_idx, line in enumerate(lines):
                if ':' in line: # Common for "Atom Symbol : Charge"
                    try: charges.append(float(line.split(':')[-1].strip()))
                    except ValueError: logger.warning(f"Could not parse charge from .txt line: '{line.strip()}' in {charge_file}")
                elif line_idx < 20 and len(line.split()) == 1: # Attempt to parse if it's just numbers (e.g. simple list)
                    try: charges.append(float(line.strip()))
                    except ValueError: pass # Ignore if not a float, might be header
        elif file_ext == '.log': # ORCA ESP charges
            esp_section_found = False
            for line in lines:
                if "ESP Fit Center  Symbol    Charge (e)" in line: esp_section_found = True; continue
                if esp_section_found:
                    if not line.strip() or "----" in line:
                        if charges: break; continue
                    parts = line.split()
                    if len(parts) >= 3: # Index, Symbol, Charge
                        try: charges.append(float(parts[-1]))
            ...
            try:
                ...
                if isinstance(data, list) and all(isinstance(item, (int, float)) for item in data):
                    ...
                    logger.warning(f"'.json' charges file {charge_file} does not contain a 'charges' list or is not a direct list of charges.")
        ...
        else: raise ValueError(f"Unsupported charges file format: {file_ext}")
        return charges if charges else None
    except FileNotFoundError:
        logger.error(f"Charges file not found: {charge_file}")
        raise
    except Exception as e: logger.error(f"Error reading charges from {charge_file}: {e}"); return None

def create_molecular_graph_json(mol_file: str, 
                                output_dir: Optional[str] = None, 
                                output_filename: Optional[str] = None,
                                config: Optional[Dict[str, Any]] = None,
                                charges_file: Optional[str] = None) -> Optional[str]:
...
def batch_create_graphs_from_molecules(mol_dir: str, 
    ...
    if not os.path.isdir(mol_dir): logger.error(f"Input directory not found: {mol_dir}"); return []
    os.makedirs(output_dir, exist_ok=True)
    mol_files = glob.glob(os.path.join(mol_dir, f"*.{file_format}"))
    if not mol_files: logger.warning(f"No .{file_format} files found in {mol_dir}"); return []
