...
def load_data():
    ...
    logger.info(f"Loading data from: {RAW_TREATMENT_PATH}")
...
def load_chemical_data():
    ...
    logger.info(f"Loading chemical data from: {CLEANED_CHEMICAL_PATH}")
...
def clean_casrn(df):
    ...
    logger.info(f"Cleaned CASRN values")
    return df
...
def convert_numeric_columns(df):
    ...
    if "Treatment_Temp_C" in df.columns:
        ...
        logger.info("Converted Treatment_Temp_C to numeric")
    ...
    if "Treatment_Time" in df.columns:
        ...
        logger.info("Converted Treatment_Time to numeric minutes")
    ...
    if "Effectiveness_Percent" in df.columns:
        ...
        logger.info("Converted Effectiveness_Percent to numeric")
    ...
    for col in concentration_columns:
        if col in df.columns:
            ...
            logger.info(f"Extracted numeric values from {col}")
...
def standardize_with_chemical_list(df, chem_df):
    ...
    logger.info("=== Standardizing Identifiers with Chemical List ===")
    ...
    logger.info(f"Treatment data has {matching_cases} matching CASRNs out of {total_cases} ({matching_cases/total_cases*100:.2f}%)")
...
def save_processed_data(df):
    ...
    # Also save using the new utility function
    output_dir = os.path.dirname(PROCESSED_TREATMENT_PATH)
    base_filename = os.path.splitext(os.path.basename(PROCESSED_TREATMENT_PATH))[0]
    
    # Use the moml save function for additional formats
    output_files = save_processed_molecules(df, output_dir, base_filename)
    logger.info(f"Saved additional formats: {', '.join(output_files.keys())}")
...
def process_treatment_data():
    ...
    try:
        ...
        logger.warning("Chemical list data not found. Skipping standardization.")
