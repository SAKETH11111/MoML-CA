...
class FeaturizeNodes:
    ...
    def __call__(self, data: Data) -> Data:
        ...
        for atom in mol.GetAtoms():
            ...
            features.extend(MolecularFeatureExtractor.one_hot_encoding(atom.GetIsAromatic(), [False, True]))
            features.extend(MolecularFeatureExtractor.one_hot_encoding(atom.IsInRing(), [False, True]))
...
class StandardizeTargets:
    ...
    def __init__(self, stats_path="data/target_stats.yaml", dataset_name="qm9"):
        try:
            ...
            raise FileNotFoundError(f"Statistics file not found: {stats_path}") from e
