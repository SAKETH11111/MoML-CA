...
def main():
    ...
    parser.add_argument(
        "--max_compounds", type=int, default=None, help="Maximum number of compounds to process (default: all)"
    )
    parser.add_argument(
        "--functional", type=str, default="B3LYP", help="Computational method/functional to use (default: B3LYP)"
    )
    parser.add_argument("--basis_set", type=str, default="STO-3G", help="Basis set to use (default: STO-3G)")
    parser.add_argument(
        "--num_procs", type=int, default=1, help="Number of processors to use per calculation (default: 1)"
    )
    parser.add_argument("--memory", type=int, default=2000, help="Memory allocation in MB (default: 2000)")
    parser.add_argument("--max_jobs", type=int, default=1, help="Maximum number of concurrent jobs (default: 1)")
    ...
    if os.path.exists(results_csv):
        ...
        try:
            ...
            # Log success rate
            success_count = (
                results_df["calculation_success"].sum() if "calculation_success" in results_df.columns else 0
            )
