...
def run_orca_calculation(input_file: str, orca_path: str = None, num_procs: int = 4) -> Tuple[bool, str]:
    ...
    try:
        ...
        process = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, cwd=input_dir)
...
def process_molecule(
    ...
    """
    results = {"id": molecule_id, "smiles": smiles, "status": "failed", "error": None, "data": None}
    ...
    success, input_file = create_orca_input(mol, molecule_id, mol_dir, functional, basis_set, num_procs, memory)
    ...
    if max_workers > 1:
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_mol):
                ...
                try:
                    ...
                    results.append({"id": molecule_id, "status": "error", "error": str(e)})
    else:
        ...
        for _, row in molecules_df.iterrows():
            ...
            result = process_molecule(
                smiles, molecule_id, output_dir, functional, basis_set, num_procs, memory, orca_path
            )
