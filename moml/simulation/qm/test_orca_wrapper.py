...
def main():
    ...
    cmd = [
        "python",
        os.path.join(CURRENT_DIR, "orca_pfas_wrapper.py"),
        "--input_csv",
        SAMPLE_DATA,
        "--output_dir",
        OUTPUT_DIR,
        "--functional",
        "B3LYP",  # Will use B3LYP-D3BJ for improved noncovalent interactions
        "--basis_set",
        "6-31G*",
        "--num_procs",
        "1",
    ...
    if is_mac_arm64:
        ...
        if os.path.exists(os.path.join(openmpi_path, "mpirun")):
            ...
            logger.warning(
                "OpenMPI installation not found. If parallel calculations fail, install OpenMPI as per the README instructions."
            )
